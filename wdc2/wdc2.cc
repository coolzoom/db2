...
/*	std::size_t pos(0);
	...
	std::unordered_map<wdc2::race,std::array<std::vector<std::uint16_t>,2>> m;
	for(auto &chr : db2.sections)
	for(const auto &ele : chr.records)
	{
		if(ele.sd)
			m[ele.cn.r].at(static_cast<std::size_t>(ele.cn.s)).emplace_back(ele.sd);
	}
	...
	for(auto &chr : db2.sections)
		for(std::size_t i(0);i!=chr.records.size();++i)
		{
			...
			{
				++count;
				const auto &f(m.at(ele.cn.r).at(static_cast<std::size_t>(ele.cn.s)));
				if(f.empty())
